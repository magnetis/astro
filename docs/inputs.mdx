...
Inputs used for messaging and similar contexts. These inputs come with embedded [ghost buttons](/docs-buttons#ghost-buttons) as commands that send away these values and contents.

<Playground className='two-columns'>
  <div className='a-input a-input--messaging'>
    <input id='input6' type='text' placeholder='Type your message here' />
    <button className='a-btn a-btn--ghost-uranus' disabled>
      send
    </button>
  </div>
  <div className='a-input a-input--messaging'>
    <input
      ...
      defaultValue='Hi, my name is Agnes…'
    />
    <button className='a-btn a-btn--ghost-uranus' disabled>
      send
    </button>
